...
int After_Pulsing(int start_file, int num_files, std::string folder) {
	...
	for (i = start_file; i < start_file + num_files; i++) {
		...
		if (std::find(C2.begin(), C2.end(), inf) != C2.end() || std::find(C2.begin(), C2.end(), neg_inf) != C2.end() || C1.size() == 0) {
			...
			if (chi2 > 1.0) {
				...
				threshold = (4./3.)*no_devs*f2->GetParError(0);
			...
			if (no_peaks > 0) {
				...
				for (Int_t i = 0; i < peak_starts.size(); i++) {
						if (C1.at(peak_starts.at(i)) < start) {
							pulse_it = i;
						}
					}
			...
			auto legend = new TLegend(0.375, 0.55, 0.9, 0.9);
			...
			std::string plot_name = OutputDirectory + HistogramName + fileno + "_after_pulsing" + filetype_plot;

			if (i - start_file + 1 < 16) {
	...
	std::string Out1 = OutputDirectory + "specs/peaks_per_file.txt";
	...
	std::string Out2 = OutputDirectory + "specs/ap_percentage.txt";
